...
import upatras.simulationmodels.devices.highaccuracy.RefrigeratorHA;
 ...
 * @author Paris
 */
public class DumbHouse extends Building {
	...
	public final TV tv;
	public final RefrigeratorHA refrigerator;
	...
	public DumbHouse(double latitude, double longitude, double length, double width, AgentSimulationInstance asi, BasicEnviroment parent_enviroment) {
		super(latitude, longitude, length, width, asi, new BasicEnviroment("House#" + houseid, parent_enviroment, Volume.cubicMeters(width * length), Temperature.degreecelsius(27), HeatAbsorbingMaterial.air().getSpecificHeatM(width * length) + HeatAbsorbingMaterial.brick().getSpecificHeatM(Math.cbrt(width * length)),null,null));
		houseid++;
	...
	public void addMember(HumanAgent human) {
		if (members.size() < max_size) {
			...
			try {
				...
				Logger.getLogger(DumbHouse.class.getName()).log(Level.SEVERE, null, ex);
			}
		}
